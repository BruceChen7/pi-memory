...
export async function callCheapModel(
  ...
  try {
    ...
    if (apiType === 'openai-completions' || apiType === 'openai-responses' || apiType === 'openai') {
      return await callOpenAICompatible(baseUrl, resolvedApiKey, modelId, prompt, combinedSignal);
    }
    if (apiType === 'anthropic-messages' || apiType === 'anthropic') {
      return await callAnthropic(baseUrl, resolvedApiKey, modelId, prompt, combinedSignal);
    }
    if (apiType === 'google-generativelanguage' || apiType === 'google') {
      return await callGoogle(baseUrl, resolvedApiKey, modelId, prompt, combinedSignal);
    }
    // Legacy support for simple types
    if (apiType === 'deepseek' || apiType === 'moonshot' || apiType === 'qwen') {
      return await callOpenAICompatible(baseUrl, resolvedApiKey, modelId, prompt, combinedSignal);
    }
  } finally {
...
async function callOpenAICompatible(
  ...
  if (!response.ok) {
    console.debug('[pi-memory] OpenAI compatible API error:', response.status, await response.text());
    return null;
  }
...
async function callAnthropic(
  ...
  if (!response.ok) {
    console.debug('[pi-memory] Anthropic API error:', response.status, await response.text());
    return null;
  }
...
async function callGoogle(
  ...
  if (!response.ok) {
    console.debug('[pi-memory] Google API error:', response.status, await response.text());
    return null;
  }
