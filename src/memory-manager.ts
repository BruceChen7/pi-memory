...
function resolvePilotAppDir(): string {
  switch (process.platform) {
    case 'win32':
      return path.join(process.env.APPDATA || path.join(homedir(), 'AppData', 'Roaming'), '.pilot');
    case 'linux':
      return path.join(process.env.XDG_CONFIG_HOME || path.join(homedir(), '.config'), '.pilot');
    default:
